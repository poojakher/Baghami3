...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\Code\\psycho\\baste\\swahili_lastrun.py',
...
# Set experiment start values for variable component i
i = 0
iContainer = []
...
study_size_text = visual.TextStim(win=win, name='study_size_text',
    text='default text',
    font='Arial',
    pos=(0.5, 0.18), height=0.05, wrapWidth=None, ori=0, 
...
for thisInstr_loop in instr_loop:
    ...
    instruction.setImage(conditions[expInfo['condition']][instr_num])
    ...
    while continueRoutine:
        ...
        if done_with_instructions.status == STARTED:
            if bool(continueRoutine==False):
                # keep track of stop time/frame for later
                done_with_instructions.tStop = t  # not accounting for scr refresh
                done_with_instructions.frameNStop = frameN  # exact frame index
                win.timeOnFlip(done_with_instructions, 'tStopRefresh')  # time at next scr refresh
                done_with_instructions.status = FINISHED
    ...
    """if instruction_count > 1, decrement count, increment img,
    repeat routine. otherwise, finish the routine and move on."""
    ...
    if iteration >= len(conditions[expInfo['condition']]):
        instr_loop.finished=True
    else:
        iteration += 1
...
for thisTrial in trials:
    ...
    for thisChoice in choices:
        ...
        study_size_text.setText(("cards: "+str(len(study))))
        test_size_text.setText(("cards: "+str(len(test))))
        done_size_text.setText(("cards: "+str(size_at_start-len(waiting))))
        unsorted_size_text.setText(("cards: "+str(len(waiting)-i)))
        # keep track of which components have finished
        choiceComponents = [choose_pile, unsorted_card, study_pile, test_pile, done_pile, study_pile_text, test_pile_text, done_pile_text, unsorted_vocab, study_size_text, test_size_text, done_size_text, unsorted_size_text, instr_choice]
        ...
        if card_to_show == "" and unsorted_card.contains(choose_pile.getPos()):
            card_to_show=str(waiting[i].swahili+" - "+waiting[i].english)
        elif card_to_show != "":
    ...
    for thisLearn_loop in learn_loop:
        ...
        # set up handler to look after randomisation of conditions etc
        study_loop = data.TrialHandler(nReps=len(active_cards), method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=[None],
        ...
        for thisStudy_loop in study_loop:
            ...
            if learn_method != "study" or len(study)==0:
                ...
                card_to_show = str(study[active_cards[card_index]].swahili+
                " - "+study[active_cards[card_index]].english)
                thisExp.addData("card_to_show",card_to_show)
        ...
        # completed len(active_cards) repeats of 'study_loop'
        ...
        # set up handler to look after randomisation of conditions etc
        test_loop = data.TrialHandler(nReps=len(active_cards), method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=[None],
        ...
        for thisTest_loop in test_loop:
            ...
            if learn_method != "test" or len(test)==0:
                ...
                card_index = randint(len(active_cards))
                card_to_show = test[active_cards[card_index]].swahili
            ...
            if learn_method != "test" or len(test)==0:
                ...
                if userinput == test[active_cards[card_index]].english:
                    thisExp.addData("test_response.eval","correct")
                else:
                    thisExp.addData("test_response.eval","incorrect")
            
                #set correct text
                card_to_show = str(test[active_cards[card_index]].swahili+
                    " - "+test[active_cards[card_index]].english)
        ...
        # completed len(active_cards) repeats of 'test_loop'
...
while continueRoutine:
    ...
    if countdown.status == STARTED:  # only update if drawing
        countdown.setText(str(timedelta(seconds=(timer_length-(datetime.now()-start).seconds))), log=False)
